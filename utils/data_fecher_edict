...
class DataFetcher_edict:
    ...
    def fetch_table_to_csv(
        ...
        """
        从数据库下载整张表（或指定字段+条件），保存为 CSV 文件。

        Args:
            table_name (str): 要下载的表名（例如 'md_trade_cal'）
            columns (Optional[List[str]]): 要下载的字段列表，如 ['CALENDAR_DATE', 'EXCHANGE_CD']。
                                          若为 None，则下载所有字段。
            where_clause (str): 可选的 WHERE 条件（不含 'WHERE' 关键字），如 "TRADE_DATE >= %s AND TRADE_DATE <= %s"
            where_params (tuple): WHERE 条件对应的参数，与 SQL 占位符匹配
            chunksize (int): 每次读取的行数，用于流式处理大表

        Returns:
            bool: 是否成功保存
        """
        self.logger.info(f"开始下载表: {table_name}")

        # 获取字段列表
        if columns is None:
            columns = self._get_table_columns(table_name)
            if not columns:
                self.logger.error(f"无法获取表 {table_name} 的字段列表，取消下载。")
                return False
            self.logger.info(f"未指定字段，将下载全部 {len(columns)} 个字段: {columns[:5]}{'...' if len(columns) > 5 else ''}")
        else:
            # 验证字段是否存在（可选，提升健壮性）
            all_cols = set(self._get_table_columns(table_name))
            invalid = [c for c in columns if c not in all_cols]
            if invalid:
                self.logger.warning(f"以下字段在表 {table_name} 中不存在: {invalid}")
        ...
        if where_clause:
            query = base_sql + " WHERE " + where_clause
            params = where_params
        else:
            query = base_sql
            params = ()

        self.logger.info(f"执行查询: {query[:200]}{'...' if len(query) > 200 else ''}")
        ...
        try:
            ...
            with self.engine.connect().execution_options(stream_results=True) as conn:
                ...
                for chunk in chunks:
                    ...
                    if num_chunks == 1:
                        self.logger.info(f"首块 {len(chunk):,} 行已写入，耗时 {time.time() - t0:.2f}s")

                t_all = time.time() - t0
                self.logger.info(
                    f"✅ 表 {table_name} 下载完成！共 {total_rows:,} 行，{num_chunks} 块，总耗时 {t_all:.2f}s"
                )
                self.logger.info(f"文件已保存至: {file_path}")
                return True

        except Exception as e:
            self.logger.error(f"下载表 {table_name} 失败: {str(e)}\n{traceback.format_exc()}")
            return False
...
if __name__ == "__main__":
    ...
    fetcher = DataFetcher_edict(data_dir)

    # ================================
    # 🟢 在这里填写你要下载的表名和字段！
    # ================================
    TABLE_NAME = "NI_News"          # ←←← 替换为你的表名
    COLUMNS = ["ID", "InfoPublDate", "Category", "InfoTitle"]           # ←←← 替换为你需要的字段，或设为 None 下载全部
    # WHERE 条件（可选）
    WHERE_CLAUSE = "TRADE_DATE >= %s AND TRADE_DATE <= %s"
    WHERE_PARAMS = ('20240101', '20241231')

    # 测试获取表字段列表
    cols = fetcher._get_table_columns("NI_News")
    print("NI_News 表的字段:", cols)

    # 执行下载
    success = fetcher.fetch_table_to_csv(
